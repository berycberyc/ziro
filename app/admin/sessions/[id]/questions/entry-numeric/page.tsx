...
export default function NumericQuestionEntryPage() {
  const params = useParams();
  const router = useRouter();
  const searchParams = useSearchParams();
  const sessionId = params.id as string;
  const variant = parseInt(searchParams.get("variant") ?? "1", 10);
  const subject = (searchParams.get("subject") ?? "rfmsh") as SubjectKey;
  ...
  const loadQuestion = useCallback(
    async (num: number) => {
      const { data } = await supabase
        .from("questions")
        .select("topic_id, text_kk, text_ru, image_url, correct_answer")
  ...
  function handleSameLangToggle(checked: boolean) {
    setSameLang(checked);
    if (checked) setTextRu(textKk);
  }

  function validate(): string | null {
    if (!topicId) return "Тема таңдалмаған.";
    if (!textKk.trim() || (!sameLang && !textRu.trim())) return "Сұрақ мәтіні толтырылмаған.";
    if (!correctAnswer.trim() || !/^\d+$/.test(correctAnswer.trim())) {
      return "Дұрыс жауап — теріс емес бүтін сан болуы керек.";
    }
    return null;
  }
    ...
    const { error: saveErr } = await supabase.from("questions").upsert(
      {
        ...
        topic_id: topicId,
        text_kk: textKk,
        text_ru: sameLang ? textKk : textRu,
        image_url: imageUrl,
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <div className="mt-6 flex flex-col gap-5 rounded-2xl border border-ink/10 bg-white p-6">
        ...
        <div>
          ...
          {imageUrl && (
            <div className="mt-2 flex items-center gap-3">
              ...
              <button onClick={() => setImageUrl(null)} className="focus-ring text-xs text-red-500 hover:underline">
                Өшіру
              </button>
            </div>
          )}
        ...
        <div>
          <label className="text-xs font-semibold text-ink/50">Дұрыс жауап (теріс емес бүтін сан)</label>
          <input
            value={correctAnswer}
            onChange={(e) => setCorrectAnswer(e.target.value.replace(/[^\d]/g, ""))}
            inputMode="numeric"
            className="focus-ring mt-1 w-40 rounded-xl border border-ink/15 px-3 py-2 text-sm"
          />
        </div>
