...
export default function AdminCurrentTestingPage() {
  ...
  return (
    <div>
      <h1 className="font-display text-2xl font-bold text-ink">Ағымдағы тестілеу</h1>

      {loading && <p className="mt-6 text-sm text-ink/50">Жүктелуде...</p>}
      {!loading && sessions.length === 0 && (
        <p className="mt-6 text-sm text-ink/50">Тексеру режиміндегі сессия жоқ.</p>
      )}
      ...
      <div className="mt-6 flex flex-col gap-4">
        {sessions.map((s) => (
          <Link
            key={s.id}
            href={`/admin/current-testing/${s.id}`}
          ...
          >
            <div>
              <p className="font-display text-lg font-bold text-ink">{s.title_kk}</p>
              <p className="text-sm text-ink/50">{s.session_date}</p>
            </div>
            <div className="flex gap-4 text-sm">
              <span className="rounded-full bg-admin-soft px-3 py-1.5 font-semibold text-admin">
                Брондалған: {s.booked}
              </span>
